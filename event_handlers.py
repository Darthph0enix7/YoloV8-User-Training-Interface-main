...
def handle_detection_page_events(event, values, window, yolo_thread, output_queue):
    if event == 'Process':
        ...
        Show_labels = 'false' if 'Show labels' in selected_options else 'true'
        Show_confidence = 'false' if 'Show confidence' in selected_options else 'true'


        command = f'yolo task=classify mode=predict save=True model="{model_path}" source="{video_path}" device={device} show={show} show_labels={Show_labels} show_conf={Show_confidence}'
